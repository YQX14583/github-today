...
import { findCachedSearchRepository } from "../../../../lib/search";
...
export default async function SearchRepositoryPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const repository = await findCachedSearchRepository(slug);
  if (!repository) notFound();

  return <>
    <nav className="site-nav"><div className="nav-inner">
    ...
    <main className="article-page">
      <Link className="back-link" href="/search">← 返回搜索</Link>
